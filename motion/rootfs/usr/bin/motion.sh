...
process_config_camera_v4l2()
{
  ...
  motion.log.trace "Set v4l2_palette to ${value}"
...
process_config_mqtt()
{
  ...
  motion.log.trace "Using MQTT host: ${value}"
  json='{"host":"'"${value}"'"'
...
motion.log.trace "setting device ${VALUE}"
MOTION_DEVICE="${VALUE}"
...
motion.log.trace "setting group ${VALUE}"
MOTION_GROUP="${VALUE}"
...
motion.log.trace "setting client ${VALUE}"
MOTION_CLIENT="${VALUE}"
...
motion.log.trace "Set motion.log_type to ${VALUE}"
...
motion.log.trace "Set log_file to ${VALUE}"
...
motion.log.trace "Set share_dir to ${VALUE}"
...
motion.log.trace "Set post_pictures to ${VALUE}"
...
motion.log.trace "Set movie_output to ${VALUE}"
...
motion.log.trace "Set text_changes to ${VALUE}"
...
motion.log.trace "Set text_scale to ${VALUE}"
...
motion.log.trace "Set despeckle_filter to ${VALUE}"
...
if [ "${VALUE}" == "null" ] || [ -z "${VALUE}" ] || [ ${VALUE:-0} == 0 ]; then 
  ...
  if [ "${VALUE}" == "null" ] || [ -z "${VALUE}" ]; then 
    VALUE=10
    motion.log.trace "DEFAULT threshold_percent to ${VALUE}"
    MOTION="${MOTION}"',"threshold_percent":'"${VALUE}"
    VALUE=$((VALUE * WIDTH * HEIGHT / 100))
  fi
else
  motion.log.trace "Set threshold_percent to ${VALUE}"
  MOTION="${MOTION}"',"threshold_percent":'"${VALUE}"
  VALUE=$((VALUE * WIDTH * HEIGHT / 100))
fi
# set threshold
...
motion.log.trace "Set interval to ${VALUE}"
...
if [ "${USERNAME}" != "null" ] && [ "${PASSWORD}" != "null" ] && [ ! -z "${USERNAME}" ] && [ ! -z "${PASSWORD}" ]; then
  motion.log.trace "Set authentication to Basic for both stream and webcontrol"
  ...
  motion.log.trace "Enable access for any host"
  ...
  motion.log.trace "WARNING: no username and password; stream and webcontrol limited to localhost only"
...
for (( i=0; i < ncamera; i++)); do
  ...
  case "${VALUE}" in
    ...
    ftpd|mqtt)
        ...
        motion.log.trace "Set mjpeg_url to ${VALUE}"
        CAMERAS="${CAMERAS}"',"mjpeg_url":"'"${VALUE}"'"'

        # icon
        VALUE=$(jq -r '.cameras['${i}'].icon' "${CONFIG_PATH}")
        if [ "${VALUE}" != "null" ] || [ ! -z "${VALUE}" ]; then 
          motion.log.trace "Set icon to ${VALUE}"
