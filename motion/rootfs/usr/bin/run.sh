...
hzn.log.debug $(date) "$0 $*"

if [ ! -s "${CONFIG_PATH}" ]; then
  hzn.log.debug "Cannot find options ${CONFIG_PATH}; exiting"
  exit
fi
...
hzn.log.debug "Setting device ${VALUE} [MOTION_DEVICE]"
...
hzn.log.debug "Setting www ${VALUE}"
JSON="${JSON}"',"www":"'"${VALUE}"'"'
...
hzn.log.debug "Setting group ${VALUE} [MOTION_GROUP]"
...
hzn.log.debug "Setting TIMEZONE ${VALUE}"
...
hzn.log.debug "Using MQTT at ${VALUE}"
MQTT='{"host":"'"${VALUE}"'"'
...
if [ ! -z "${WVR_URL}" ] && [ ! -z "${WVR_APIKEY}" ] && [ ! -z "${WVR_DATE}" ] && [ ! -z "${WVR_VERSION}" ] && [ "${WVR_URL}" != "null" ] && [ "${WVR_APIKEY}" != "null" ] && [ "${WVR_DATE}" != "null" ] && [ "${WVR_VERSION}" != "null" ]; then
  hzn.log.debug "Watson Visual Recognition at ${WVR_URL} date ${WVR_DATE} version ${WVR_VERSION}"
  ...
  if [ ! -z "${WVR_CLASSIFIER}" ] && [ "${WVR_CLASSIFIER}" != "null" ]; then
    ...
    hzn.log.debug "Using classifiers(s): ${CLASSIFIERS}"
...
if [ "${VALUE}" != "null" ] && [ ! -z "${VALUE}" ]; then
  ...
  if [ ! -z "${VALUE}" ] && [ "${VALUE}" != "null" ]; then
    ...
    hzn.log.debug "Using DIGITS at ${DIGITS_SERVER_URL} and ${DIGITS_JOBIDS}"
...
hzn.log.debug "Set log_type to ${VALUE}"
...
hzn.log.debug "Set output_pictures to ${VALUE}"
...
hzn.log.debug "Set motion log_level to ${VALUE}"
...
if [ "${VALUE}" != "null" ] && [ ! -z "${VALUE}" ]; then
  hzn.log.debug "Set v4l2_pallette to ${VALUE}"
...
hzn.log.debug "Set quality to ${VALUE}"
...
if [ "${VALUE}" == "null" ] || [ -z "${VALUE}" ]; then VALUE=0; fi
hzn.log.debug "Set minimum_frame_time to ${VALUE}"
...
hzn.log.debug "*** Found ${ncamera} cameras"
...
for (( i=0; i<ncamera ; i++)) ; do
  ...
  hzn.log.debug "CAMERA #: $i CONF: ${MOTION_CONF} NUM: $CNUM"
  ...
  hzn.log.debug "Set fps to ${VALUE}"
  ...
  hzn.log.debug "Set netcam_url to ${NETCAM_URL}"
...
if [ ! -s "${MOTION_JSON_FILE}" ]; then
  hzn.log.debug "Invalid JSON: ${JSON}"
  exit
else
  hzn.log.debug "Publishing configuration to ${MOTION_MQTT_HOST} topic ${MOTION_GROUP}/${MOTION_DEVICE}/start"
...
if [ "${URL}" != "null" ] && [ "${USERNAME}" != "null" ] && [ "${PASSWORD}" != "null" ] && [ ! -z "${URL}" ] && [ ! -z "${USERNAME}" ] && [ ! -z "${PASSWORD}" ]; then
  hzn.log.debug "Testing CLOUDANT"
  OK=$(curl -sL "${URL}" | jq '.couchdb?!=null')
  if [ "${OK}" == "false" ]; then
    hzn.log.debug "Cloudant failed at ${URL}; exiting"
    exit
  else
    export MOTION_CLOUDANT_URL="${URL%:*}"'://'"${USERNAME}"':'"${PASSWORD}"'@'"${USERNAME}"."${URL#*.}"
    hzn.log.debug "Cloudant succeeded at ${MOTION_CLOUDANT_URL}; exiting"
  fi
  ...
  if [ "${DB}" != "${MOTION_GROUP}" ]; then
    ...
    if [ "${OK}" != "true" ]; then
      hzn.log.debug "Failed to create CLOUDANT DB ${MOTION_GROUP}"
      OFF=TRUE
    else
      hzn.log.debug "Created CLOUDANT DB ${MOTION_GROUP}"
    fi
  else
  ...
  if [ -s "${MOTION_JSON_FILE}" ] && [ -z "${OFF:-}" ]; then
    ...
    if [ "${REV}" != "null" ] && [ ! -z "${REV}" ]; then
      hzn.log.debug "Prior record exists ${REV}"
      URL="${URL}?rev=${REV}"
    fi
    OK=$(curl -sL "${URL}" -X PUT -d "@${MOTION_JSON_FILE}" | jq '.ok')
    if [ "${OK}" != "true" ]; then
      hzn.log.debug "Failed to update ${URL}" $(jq -c '.' "${MOTION_JSON_FILE}")
      hzn.log.debug "Exiting"; exit
    else
      hzn.log.debug "Configuration for ${MOTION_DEVICE} at ${MOTION_JSON_FILE}" $(jq -c '.' "${MOTION_JSON_FILE}")
    fi
  else
    hzn.log.debug "Failed; no DB or bad JSON"
    hzn.log.debug "Exiting"; exit
  fi
else
  hzn.log.debug "Cloudant URL, username and/or password undefined"
fi
if [ -z "${MOTION_CLOUDANT_URL:-}" ]; then
  hzn.log.debug "Cloudant NOT SPECIFIED"
fi

# test hassio
hzn.log.debug "Testing hassio ..." $(curl -sL -H "X-HASSIO-KEY: ${HASSIO_TOKEN}" "http://hassio/supervisor/info" | jq -c '.')
# test homeassistant
hzn.log.debug "Testing homeassistant ..." $(curl -sL -u ":${HASSIO_TOKEN}" "http://hassio/homeassistant/api/states" | jq -c '.')
...
if [ "${VALUE}" != "false" ]; then 
  hzn.log.debug "Re-defining configuration YAML (${VALUE})"
  JSON="${JSON}"',"reload":'"${VALUE}"
  rlyaml.sh "${VALUE}"
else
  hzn.log.debug "Not re-defining configuration YAML (${VALUE})"
fi
...
if [ ! -s "${MOTION_CMD}" ] || [ ! -s "${MOTION_CONF}" ]; then
  hzn.log.debug "No motion installed (${MOTION_CMD}) or motion configuration ${MOTION_CONF} does not exist"
else
...
if [ -s "${MOTION_APACHE_CONF}" ]; then
  ...
  hzn.log.debug "Starting Apache: ${MOTION_APACHE_CONF} ${MOTION_APACHE_HOST} ${MOTION_APACHE_PORT} ${MOTION_APACHE_HTDOCS}"
