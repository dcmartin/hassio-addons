...
if ( -e /usr/bin/dateutils.dconv ) then
  ...
  if ($?USE_MQTT && $?DEBUG) mosquitto_pub -h "${MOTION_MQTT_HOST}" -t "debug" -m '{"ERROR":"'$0:t'","pid":'$$',"error":"no date converter; install dateutils"}'
...
if ($?USE_MQTT && $?VERBOSE) mosquitto_pub -h "${MOTION_MQTT_HOST}" -t "debug" -m '{"VERBOSE":"'$0:t'","pid":'$$',"dir":"'${dir}'","camera":"'$CN'","event":"'$EN'","jsons":'$#jsons'}'
...
if ($#jsons) then
  ...
  if (! -s "$lastjson") then
    if ($?USE_MQTT && $?DEBUG) mosquitto_pub -h "${MOTION_MQTT_HOST}" -t "debug" -m '{"DEBUG":"'$0:t'","pid":'$$',"dir":"'${dir}'","camera":"'$CN'","event":"'$EN'","error":"'"$lastjson:t"' is empty"}'
  ...
  if ($?USE_MQTT && $?DEBUG) mosquitto_pub -h "${MOTION_MQTT_HOST}" -t "debug" -m '{"DEBUG":"'$0:t'","pid":'$$',"dir":"'${dir}'","camera":"'$CN'","event":"'$EN'","error":"no JSONS '"$?jsons"'"}'
  if ($?DEBUG) echo "$0:t $$ -- Found no JSON; exiting" >& /dev/stderr
  goto done
endif

if ($?USE_MQTT && $?VERBOSE) mosquitto_pub -h "${MOTION_MQTT_HOST}" -t "debug" -m '{"VERBOSE":"'$0:t'","pid":'$$',"dir":"'${dir}'","camera":"'$CN'","event":"'$EN'","candidates":'"$#jpgs"'}'
...
if ($#jpgs) then
  ...
  while ($i > 0) 
    ...
    set jsn = "$jpg:r.json"
    if ( ! -s "$jsn" ) then
      if ($?USE_MQTT && $?DEBUG) mosquitto_pub -h "${MOTION_MQTT_HOST}" -t "debug" -m '{"DEBUG":"'$0:t'","pid":'$$',"camera":"'$CN'","jpg":"'$jpg'","error":"cannot find JSON ('"$jsn"')"}'
...
if ($?USE_MQTT && $?VERBOSE) mosquitto_pub -h "${MOTION_MQTT_HOST}" -t "debug" -m '{"VERBOSE":"'$0:t'","pid":'$$',"camera":"'$CN'","event":"'${EN}'","elapsed":'$elapsed',"images":'"$images"'}'
...
if ( "$JSON" == "" ) then
  if ($?USE_MQTT && $?DEBUG) mosquitto_pub -h "$MOTION_MQTT_HOST" -t "debug" -m '{"DEBUG":"'$0:t'","pid":'$$',"camera":"'$CN'","event":"'"$EN"'","FAILED":"'"$lastjson"'"}'
  if ($?DEBUG) echo "$0:t $$ -- Failed to update JSON $lastjson; exiting: $JSON" >& /dev/stderr
  goto done
else
  if ($?USE_MQTT && $?VERBOSE) mosquitto_pub -h "${MOTION_MQTT_HOST}" -t "debug" -m '{"VERBOSE":"'$0:t'","pid":'$$',"event":'"$JSON"'}'
...
foreach f ( $frames )
  ...
  if ($?USE_MQTT && $?VERBOSE) mosquitto_pub -h "${MOTION_MQTT_HOST}" -t "debug" -m '{"VERBOSE":"'$0:t'","pid":'$$',"jpg":"'"$jpg"'","info":'"${info}"'}'
  ...
  if (-e "$jpg:r.json") then
    ...
    if (-s /tmp/$0:t.$$.json) then
      ...
      rm -f /tmp/$0:t.$$.json
      if ($?USE_MQTT && $?DEBUG) mosquitto_pub -h "${MOTION_MQTT_HOST}" -t "debug" -m '{"DEBUG":"'$0:t'","pid":'$$',"jpg":"'"$jpg"'","error":"JSON failed"}'
      if ($?DEBUG) echo "$0:t $$ -- JSON failed; skipping" >& /dev/stderr
      continue
    endif
  else
    if ($?USE_MQTT && $?DEBUG) mosquitto_pub -h "${MOTION_MQTT_HOST}" -t "debug" -m '{"DEBUG":"'$0:t'","pid":'$$',"jpg":"'"$jpg"'","error":"JSON not found"}'
    if ($?DEBUG) echo "$0:t $$ -- Failed to find JSON $jpg:r.json; skipped" >& /dev/stderr
  endif
end

if ($#jpgs) then
  if ($?USE_MQTT && $?VERBOSE) mosquitto_pub -h "${MOTION_MQTT_HOST}" -t "debug" -m '{"VERBOSE":"'$0:t'","pid":'$$',"njpgs":'$#jpgs'}'
  if ($?VERBOSE) echo "$0:t $$ -- Found $#jpgs images from $#frames frames" >& /dev/stderr
else
  if ($?USE_MQTT && $?DEBUG) mosquitto_pub -h "${MOTION_MQTT_HOST}" -t "debug" -m '{"DEBUG":"'$0:t'","pid":'$$',"error":"insufficient jpegs"}'
...
if ($#jpgs > 1) then
  ...
  if ( -s "$average") then
    ...
    set out = `cat "/tmp/$$.out"`
    if ($?USE_MQTT && $?DEBUG) mosquitto_pub -h "${MOTION_MQTT_HOST}" -t "debug" -m '{"DEBUG":"'$0:t'","pid":'$$',"error":"'"$out"'"}'
  ...
  while ( $i <= $#jpgs )
    ...
    if ($?USE_MQTT && $?VERBOSE) mosquitto_pub -h "${MOTION_MQTT_HOST}" -t "debug" -m '{"VERBOSE":"'$0:t'","pid":'$$',"jpg":"'"$jpgs[$i]"'","size":"'$size'"}'
    # keep track of size
    @ totalsize += $size
  ...
  if ($?USE_MQTT && $?VERBOSE) mosquitto_pub -h "$MOTION_MQTT_HOST" -t "debug" -m '{"VERBOSE":"'$0:t'","pid":'$$',"diff":'$avgdiff',"size":'$avgsize'}'
  ...
  if ($?USE_MQTT && $?DEBUG) mosquitto_pub -h "${MOTION_MQTT_HOST}" -t "debug" -m '{"DEBUG":"'$0:t'","pid":'$$',"warn":"singleton"}'
endif

### GET post_pictures
set post_pictures = `jq -r '.post_pictures' "$MOTION_JSON_FILE"`
if ($?USE_MQTT && $?VERBOSE) mosquitto_pub -h "$MOTION_MQTT_HOST" -t "debug" -m '{"VERBOSE":"'$0:t'","pid":'$$',"post_pictures":"'"$post_pictures"'"}'
...
if ($#IF && -s "$IF" && $?MOTION_MQTT_HOST && $?MOTION_MQTT_PORT) then
  ...
  if ($?USE_MQTT && $?DEBUG) mosquitto_pub -h "${MOTION_MQTT_HOST}" -t "debug" -m '{"DEBUG":"'$0:t'","pid":'$$',"post":"'"$post_pictures"'","topic":"'"$MQTT_TOPIC"'","image":"'${IF}'"}'
...
if ($?USE_MQTT && $?VERBOSE) mosquitto_pub -h "${MOTION_MQTT_HOST}" -t "debug" -m '{"VERBOSE":"'$0:t'","pid":'$$',"minimum_animate":"'"$minimum_animate"'"}'
...
if ($minimum_animate <= $#jpgs && $#jpgs > 1) then
  ...
  if ($#fps == 0) then
    if ($?USE_MQTT && $?DEBUG) mosquitto_pub -h "${MOTION_MQTT_HOST}" -t "debug" -m '{"DEBUG":"'$0:t'","pid":'$$',"WARN":"camera '"$CN"' not found in '"${MOTION_JSON_FILE}"'"}'
  ...
  set delay = $delay:r
  if ($?USE_MQTT && $?VERBOSE) mosquitto_pub -h "${MOTION_MQTT_HOST}" -t "debug" -m '{"VERBOSE":"'$0:t'","pid":'$$',"FPS":'$fps',"delay":'$delay'}'
  ...
  if (! -s "$gif") then
    if ($?USE_MQTT && $?DEBUG) mosquitto_pub -h "${MOTION_MQTT_HOST}" -t "debug" -m '{"DEBUG":"'$0:t'","pid":'$$',"gif":"FAILED"}'
  ...
  popd >& /dev/null
  if (! -s "$mask") then
    if ($?USE_MQTT && $?DEBUG) mosquitto_pub -h "${MOTION_MQTT_HOST}" -t "debug" -m '{"DEBUG":"'$0:t'","pid":'$$',"mask":"FAILED"}'
...
##
  ...
  if ($?USE_MQTT && $?VERBOSE) mosquitto_pub -h "${MOTION_MQTT_HOST}" -t "debug" -m '{"VERBOSE":"'$0:t'","pid":'$$',"info":"END"}'
