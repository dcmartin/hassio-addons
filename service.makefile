...
SERVICE_PORT ?= $(shell jq -r '.ports?' config.json | sed 's/[0-9]*[:]*\([0-9]*\).*/\1/' | sed 's/null//')
...
build: Dockerfile build.json config.json makefile remove
...
service-start: start-service
	@echo "${MC}>>> MAKE --" $$(date +%T) "-- started service: ${SERVICE_NAME}""${NC}" > /dev/stderr

start-service: stop stop-service depend
	@echo "${MC}>>> MAKE --" $$(date +%T) "-- starting service: ${SERVICE_NAME}""${NC}" > /dev/stderr
	-@docker start "${DOCKER_NAME}" > /dev/null

start: service-start

service-stop: stop-service
	@echo "${MC}>>> MAKE --" $$(date +%T) "-- service-stop: ${SERVICE_NAME}""${NC}" > /dev/stderr

stop-service: 
	@echo "${MC}>>> MAKE --" $$(date +%T) "-- stop-service: ${SERVICE_NAME}""${NC}" > /dev/stderr
...
TEST_RESULT = ./test.${BUILD_ARCH}_${SERVICE_ID}_${SERVICE_VERSION}.out
TEST_OUTPUT = ./test.${BUILD_ARCH}_${SERVICE_ID}_${SERVICE_VERSION}.json
