...
# environment
if ($?HORIZON_DEVICE_DB == 0) then
  set HORIZON_DEVICE_DB = "$APP"
  if ($?DEBUG) echo `date` "$0:t $$ -- HORIZON_DEVICE_DB not set; using $HORIZON_DEVICE_DB" >>&! $LOGTO
endif
if ($?HORIZON_CLOUDANT_URL == 0) then
...
# find devices
if ($db == "all") then
  set devices = ( `curl -s -q -L "$CU/$HORIZON_DEVICE_DB/_all_docs" | jq -r '.rows[]?.id'` )
  if ($#devices == 0) then
...
foreach d ( $devices )
  # get device entry
  set url = "$CU/$HORIZON_DEVICE_DB/$d"
