...
# environment
if ($?HORIZON_SHARE_DIR == 0) then
  set HORIZON_SHARE_DIR = "/share/$APP"
  if ($?DEBUG) echo `date` "$0:t $$ -- environment error: HORIZON_SHARE_DIR undefined; using $HORIZON_SHARE_DIR" >>&! $LOGTO
endif
...
## ACCESS CLOUDANT
##
if ($?HORIZON_CLOUDANT_URL) then
  set CU = "${HORIZON_CLOUDANT_URL}"
...
## ACCESS WATSON_VR
##
if ($?ADDON_CONFIG_FILE == 0) then
  if ($?DEBUG) echo `date` "$0:t $$ -- ADDON_CONFIG_FILE undefined" >>&! $LOGTO
  goto done
else if (! -s "$ADDON_CONFIG_FILE") then
  if ($?DEBUG) echo `date` "$0:t $$ -- ADDON_CONFIG_FILE ($ADDON_CONFIG_FILE) does not exist or zero size" >>&! $LOGTO
  goto done
else
  set WVR_URL = `jq -r '.watson.url' "$ADDON_CONFIG_FILE"`
  set WVR_DATE = `jq -r '.watson.date' "$ADDON_CONFIG_FILE"`
  set WVR_VERSION = `jq -r '.watson.version' "$ADDON_CONFIG_FILE"`
endif

if ($?WVR_URL == 0 || $?HORIZON_WATSON_APIKEY == 0 || $?WVR_DATE == 0 || $?WVR_VERSION == 0) then
  if ($?DEBUG) echo `date` "$0:t $$ -- insufficient Watson credentials in $ADDON_CONFIG_FILE" >>&! $LOGTO
  goto done
else if ($#WVR_URL == 0 || $#HORIZON_WATSON_APIKEY == 0 || $#WVR_DATE == 0 || $#WVR_VERSION == 0) then
  if ($?DEBUG) echo `date` "$0:t $$ -- incomplete Watson credentials in $ADDON_CONFIG_FILE" >>&! $LOGTO
  goto done
endif

if ($?VERBOSE) echo `date` "$0:t $$ -- Watson credentials found (url $#WVR_URL key $#HORIZON_WATSON_APIKEY date $#WVR_DATE version $#WVR_VERSION)" >>&! $LOGTO

# find models and dbs
set CLASSIFIERS = "$TMP/$APP-$API-classifiers.$DATE.json"
if (! -s "$CLASSIFIERS") then
  rm -f "$TMP/$APP-$API-classifiers".*.json
  curl -q -s -L "$WVR_URL/$WVR_VERSION/classifiers?api_key=$HORIZON_WATSON_APIKEY&version=$WVR_DATE" >! "$CLASSIFIERS"
endif
...
if ($#classifiers == 0) then
  ...
  set OUTPUT = "$HORIZON_SHARE_DIR/matrix/$model.json"
  if (-s "$OUTPUT") then
    echo "Content-Type: application/json; charset=utf-8"
    echo "Access-Control-Allow-Origin: *"
